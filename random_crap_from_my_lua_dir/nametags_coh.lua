...
local enabled_cl = CreateClientConVar(tag .. "_enabled", "1", true, true, "Should nametags send your chat as you type (and display other peoples)?")
...
do
	...
	hook.Add("ChatTextChanged", tag, function(text)
		...
		local len = string.len(text) -- its binary, not utf8.len
		--if len > 4096 then return end -- loudno
	...
	hook.Add("FinishChat", tag, function()
		if not started and not enabled_cl:GetBool() then return end -- fixes a thing where disabling after opening chat leaves it there forever
		net.Start(finish)
		net.SendToServer()
