...
local function drawents()
	...
	local function drawent(entphys, realent, boneindex)
		...
		pos_axis = LocalToWorld(entphys, vec_forward):ToScreen()
		SetDrawColor( 255, 0, 0, 255 ) DrawLine( pos_axis.x, pos_axis.y, scrposx, scrposy )

		pos_axis = LocalToWorld(entphys, vec_left):ToScreen()
		SetDrawColor( 0, 255, 0, 255 ) DrawLine( pos_axis.x, pos_axis.y, scrposx, scrposy )

		pos_axis = LocalToWorld(entphys, vec_up):ToScreen()
	...
	for _, ent in ipairs(entstable) do
		if mode == 1 then
			...
			for i = 0, math.max(0, ent:GetPhysicsObjectCount() - 1) do
	...
	if IsValid(traceent) and not freeze_labels then
		local hasphys = false
		if mode == 2 then
	...
	local function drawlabel(ent,v)
		...
		if isEntity then
			...
			if not IsValid(realent) then return end
