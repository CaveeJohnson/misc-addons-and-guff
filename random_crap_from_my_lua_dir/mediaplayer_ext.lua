...
do
	...
	do
		...
		function mediaplayer_ext.queueYTPlaylist(ent, ply, pid, fail)
			...
			mediaplayer_ext.getYTPlaylistData(pid, function(vids)
				...
				for idx, v in ipairs(vids) do
					...
					if not v.videoId then
						ErrorNoHalt("// mediaplayer_ext.getYTPlaylistData HAS DONE FUCKED UP! //\n")
						PrintTable(v)
						MsgN("// END // ")
						return
					end
		...
		if aowl then
			...
			aowl.AddCommand("theaterpl", function(ply, line)
				if ply.nextYTPlaylistTime and ply.nextYTPlaylistTime > CurTime() then
					return false, "The youtube API has a quota tied to our API key, stop spamming this."
				end

				local ent = theater.screen
				if not (IsValid(ent) and ent.GetMediaPlayer) then return false, "Theater not present" end

				mediaplayer_ext.queueYTPlaylist(ent, ply, line, function(msg)
					aowl.Message(ply, msg)
				end)
				ply.nextYTPlaylistTime = CurTime() + 120
			end, "developers")
		elseif BaseWars then
			BaseWars.Commands.AddCommand({"queueplaylist", "playlist", "ytpl"}, function(ply, line)
				if ply.nextYTPlaylistTime and ply.nextYTPlaylistTime > CurTime() then
					return false, "The youtube API has a quota tied to our API key, stop spamming this."
				end
			...
			BaseWars.Commands.AddCommand("theaterpl", function(ply, line)
				if ply.nextYTPlaylistTime and ply.nextYTPlaylistTime > CurTime() then
					return false, "The youtube API has a quota tied to our API key, stop spamming this."
				end
